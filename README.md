...
Tokenizer configs are authored for the Rust `tokenizers` crate, which compiles patterns with Oniguruma — a regex engine whose syntax and Unicode behavior differ from JavaScript `RegExp` in many ways. Tokenizers.js translates these patterns structurally, matching Oniguruma semantics verified against Python `tokenizers`: line anchors (`^`/`$` recognize only `\n`, unlike JavaScript's `m` flag), absolute anchors (`\A`, `\z`, `\Z`), `.` (which excludes only `\n`), word/digit/space shorthands (including Oniguruma's exact word-character set and its Latin-1 ctype quirks), `\h`/`\H` hex digits, `\b`/`\B` boundaries, inline case-insensitive groups (including ranges like `(?i:[a-f])`), possessive and stacked quantifiers (`a++`, `X{3}+`), atomic groups, POSIX bracket expressions, `\x{...}` code points, `\p{Word}`, identity escapes, and literal braces/brackets.

The test suite compares JavaScript `Split` pre-tokenizer output against oracle fixtures generated by Python `tokenizers` (`tests/py/generate_split_regex_oracle.py`), covering both patterns sampled from the Hugging Face Hub and targeted edge cases: line-terminator handling, CRLF endings, Unicode word-character categories, hex digits, case folding, and whitespace sets.

Some behavior is a known limitation and is pinned by `knownDivergence` test cases: `\G` is dropped (its continuation semantics cannot be expressed), possessive/atomic constructs lose their backtracking restrictions (same match sets, but pathological inputs may differ), full Unicode case folding (e.g. `ß` ~ `ss`) is not performed, character-class intersection (`&&`) and nested classes are unsupported, `\W`/`\H` inside negated classes fall back to ASCII semantics, and `Split` behaviors such as `MergedWithPrevious`, `MergedWithNext`, and `Contiguous` may differ.
