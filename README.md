...
A few constructs can't be fully reproduced — notably `\G`, full Unicode case folding (e.g. `ß` ~ `ss`), unlisted POSIX classes, negated POSIX `lower`/`upper` classes inside inline case-insensitive groups, POSIX collating and equivalence bracket expressions (`[.x.]` and `[=x=]`), and the `MergedWithPrevious`/`MergedWithNext`/`Contiguous` split behaviors. Malformed or unbalanced intersections, empty operands, and character-class ranges with a set-valued start or endpoint are intentionally rejected rather than approximated. Set-valued range operands include positive or negative Unicode properties, POSIX classes, shorthand classes, nested classes, and complemented sets. Outer-negated intersections containing a nested complemented Unicode-property, POSIX, or shorthand set (for example, `[^[^\p{L}]&&[a]]`) are also rejected because their Oniguruma semantics cannot be represented safely with JavaScript `u`-mode set operations; ordinary scalar/range forms such as `[^a-z&&[^m-p]]` remain supported. Character-class nesting is limited to 256 levels. Inside a character class, a literal `&&` must be escaped as `\&\&`.
